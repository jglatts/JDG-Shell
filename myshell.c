/*
	...
	The shell program runs in a forever loop and then runs two main steps, 
	running the parser and then running the user command. It first parses 
	the user input into tokens seperated by whitespace.The parser then checks 
	the input for any file redirection, pipes, background processing, 
	or parrallel processing needs. The shell will then set any neccessary flags
	in order to produce the desired output. After parsing, the second step is running
	the user supplied command. The shell will check if either a built in or 
	and external command will have to be executed. It will continue to loop and
	exectue commands until the user enters the "exit" command.
*/
#include "myshell.h"
#include <stdio.h>
#include <stdlib.h> 
...
/*
	Calls neccessary parsing functions 
...
void check_file_io() {
	...
	for (i = 0; cmd_strings[i] != NULL; ++i) {
		...
		if (strcmp(cmd_strings[i], "&") == 0) {
			parse_parrallel_cmds(&old_pos, i);
		}
	}
}

/*
	Parser for multiple cmds to run in parrallel
	Assigns the cmds to a new array which will
	be used to run the programs in parrallel
*/
void parse_parrallel_cmds(int* old_pos, int i) {
	background = true;
	background_pos = i;
	if (multi_count == 0) {
		multi_parallel[multi_count] = (MultiParallel*)malloc(sizeof(MultiParallel));
		if (!multi_parallel[multi_count]) return;
		int j;
		int count = 0;
